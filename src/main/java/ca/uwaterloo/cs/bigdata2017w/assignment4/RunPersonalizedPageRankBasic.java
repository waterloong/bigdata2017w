...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      if (node.getAdjacenyList().size() > 0) {
        ...
        for (int i = 0; i < sources.length; i ++) {
          float mass = (float) (masses.get(i) - StrictMath.log(list.size()));
          masses.set(i, mass);
        }
  ...
  private static class CombineClass extends
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> values, Context context)
      ...
      if (massMessages > 0) {
        ...
        intermediateMass.setType(PageRankNode.Type.Mass);
        intermediateMass.setPageRanks(new ArrayListOfFloatsWritable(masses));
  ...
  private static class ReduceClass extends
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
      ...
      if (structureReceived == 1) {
        ...
        for (int i = 0; i < sources.length; i ++) {
          totalMass[i] = sumLogProbs(totalMass[i], masses[i]);
        }
  ...
  private static class MapPageRankMassDistributionClass extends
      Mapper<IntWritable, PageRankNode, IntWritable, PageRankNode> {
    private String missingMass[];
    private String[] sources;

    @Override
    public void setup(Context context) throws IOException {
      Configuration conf = context.getConfiguration();
      missingMass = conf.getStrings("MissingMass");
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      for (int i = 0; i < sources.length; i ++) {
        ...
        if (nid.get() == source) {
          float missing = StrictMath.max(Float.parseFloat(missingMass[i]),  0.0f);

          float jump = (float) StrictMath.log(ALPHA);
          float link = (float) StrictMath.log(1.0f - ALPHA)
                  + sumLogProbs(pageRanks.get(i), (float) StrictMath.log(missing));
          pageRanks.set(i, sumLogProbs(jump, link));
        } else {
          pageRanks.set(i, (float) (pageRanks.get(i) + StrictMath.log(1 - ALPHA)));
        }
